  ...
  return (
    <div className={`space-y-8 animate-in fade-in duration-700 pb-20 ${esAlerta ? 'bg-red-600/10 p-4 rounded-[3rem]' : ''}`}>
      <header className="flex justify-between items-start pt-4">
        <div>
          <h1 className="text-3xl font-black text-slate-900 tracking-tighter leading-none">Hola, {userData?.first_name || 'Papá/Mamá'}.</h1>
          <p className="text-slate-500 italic text-sm mt-1">"Si la vida te da limones, haz limonada."</p>
        </div>
      ...
      <div className={`rounded-[3rem] p-8 text-white relative overflow-hidden shadow-2xl transition-all ${esAlerta ? 'bg-red-600 animate-pulse' : 'bg-slate-900'}`}>
        ...
        <div className="relative z-10 text-center">
          <p className="text-[10px] font-black uppercase tracking-[0.2em] text-orange-400 mb-2">Tu racha actual</p>
      ...
      <div className="text-center bg-white border border-slate-100 p-6 rounded-[2.5rem] italic text-slate-600 text-sm shadow-sm font-medium">
        "Los resultados dependen de nuestras reacciones ante las acciones de los demás. No te doblegues ante la situación."
      </div>

      {/* BOTÓN SOS - INTERCEPTOR */}
      <button 
        onClick={() => setView('sos')} 
        className="w-full h-36 bg-red-600 text-white rounded-[2.5rem] flex flex-col items-center justify-center shadow-2xl shadow-red-200 animate-pulse active:scale-95 transition-all"
      >
        <Zap size={36} className="mb-1 fill-white" />
        <span className="text-2xl font-black uppercase tracking-tighter">¡SOS INTERCEPTOR!</span>
        <span className="text-[10px] font-bold opacity-80 uppercase tracking-widest">Uso inmediato antes de gritar</span>
      </button>

      {/* KIT PRINCIPAL (1-5) */}
      <div className="grid grid-cols-1 gap-4">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">Kit de Emergencia (En orden lógico)</h3>
        <ToolCard title="1. Mapa de Zonas Rojas" desc="Prevención y Energía" icon={<Map/>} onClick={() => setView('mapa')} color="bg-orange-50 text-orange-600" />
      ...
      {!esPremium ? (
        <div className="mt-12 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem] p-10 text-center space-y-6">
          ...
          <p className="text-sm text-slate-500 italic font-medium">
            "Hijo, el grito silenciado es solo el inicio. El Sistema de Influencia es el que construye el respeto real sin necesidad de alzar la voz."
          </p>
        ...
        <div className="mt-12 space-y-6 pt-10 border-t-8 border-blue-50">
          ...
          <div className="grid grid-cols-1 gap-4 bg-blue-50/40 p-6 rounded-[3rem]">
            <ToolCard title="Protocolo de Consecuencias" icon={<Gavel/>} onClick={() => setView('consecuencias')} color="bg-white text-green-600 shadow-sm" />
            ...
            <ToolCard title="Audios de Re-Programación" icon={<Headphones/>} onClick={() => setView('audios')} color="bg-white text-indigo-600 shadow-sm" />
