...
const Home = ({ userData, setView }) => {
  ...
  return (
    <div className={`space-y-8 animate-in fade-in duration-700 pb-20 ${esAlerta ? 'bg-red-600/10 p-4 rounded-[3rem]' : ''}`}>
      ...
      {/* BOTÓN SOS - INTERCEPTOR (JERARQUÍA MÁXIMA) */}
      <button 
        onClick={() => setView('sos')} 
      ...
      >
        ...
        <span className="text-[10px] font-bold opacity-80 uppercase tracking-widest italic">Emergencia: Úsalo antes de gritar</span>
      </button>

      {/* HERRAMIENTAS ESTÁNDAR (1-5) */}
      <div className="grid grid-cols-1 gap-4">
        <h3 className="text-[10px] font-black uppercase tracking-widest text-slate-400 ml-4">Kit de Emergencia (Síguelo en orden)</h3>
        ...
        <ToolCard title="5. Guía Primeros Auxilios" desc="Reparar el vínculo tras un grito" icon={<Heart/>} onClick={() => setView('auxilios')} color="bg-rose-50 text-rose-600" />
      </div>
      ...
      {!esPremium ? (
        <div className="mt-12 bg-slate-50 border-2 border-dashed border-slate-200 rounded-[3rem] p-10 text-center space-y-6">
          ...
          <p className="text-sm text-slate-500 italic font-medium">
            "Hijo, silenciar el grito es el primer paso. El segundo es construir una influencia real donde ellos elijan escucharte."
          </p>
          <button className="w-full py-6 bg-slate-900 text-white rounded-3xl font-black uppercase tracking-widest shadow-xl hover:bg-slate-800 active:scale-95 transition-all">
        ...
        <div className="mt-12 space-y-6 pt-10 border-t-8 border-blue-50">
          ...
          <div className="grid grid-cols-1 gap-4 bg-blue-50/30 p-6 rounded-[3rem]">
