...
import { Moon, Zap, Coffee, ToyBrick, Monitor, ChevronRight } from 'lucide-react';
  ...
  const scripts = {
    ...
    prisas: {
      ...
      items: [
        ...
        { f: "[Consecuencia Lógica del Retraso]", s: "El autobús no espera. ¿Quieres desayunar sentado conmigo o prefieres que guarde el pan en una bolsa para que comas en el camino?" }
      ]
    },
    comida: {
      icon: <Coffee size={18}/>,
      items: [
        ...
        { f: "[Escudos Invisibles]", s: "Esta sopa tiene escudos invisibles. Cada cucharada es un escudo contra los resfriados. ¿Cuántos quieres hoy: cinco o diez?" }
      ]
    },
    orden: {
      icon: <ToyBrick size={18}/>,
      items: [
        { f: "[Competencia 60s]", s: "Vamos a ver quién junta más piezas del suelo en 60 segundos. ¡El que gane elige el color del vaso de la cena!" },
        { f: "[Caja de Descanso]", s: "Los juguetes que se queden en el suelo se irán a mi 'Caja de Descanso' y no podrán salir hasta mañana porque están muy cansados." }
      ]
    },
    pantallas: {
      icon: <Monitor size={18}/>,
      items: [
        { f: "[Aviso Previo]", s: "En 5 minutos la tablet necesita ir a cargarse. ¿Quieres apagarla tú ahora o esperar a que se apague solita y te quedes a medias?" },
    ...
    <div className="space-y-6 pb-12 animate-in slide-in-from-bottom duration-500">
      ...
      <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar">
        {Object.keys(scripts).map(k => (
          <button 
            key={k} 
            onClick={() => setCat(k)} 
            className={`px-6 py-4 rounded-3xl text-xs font-black uppercase tracking-widest transition-all flex items-center gap-2 shrink-0 ${cat === k ? 'bg-orange-600 text-white shadow-lg' : 'bg-slate-50 text-slate-400 hover:bg-slate-100'}`}
          >
            {scripts[k].icon} {k}
          </button>
        ))}
      ...
      <div className="space-y-4">
        {scripts[cat].items.map((item, i) => (
          <div key={i} className="p-8 bg-white rounded-[3rem] border-l-[12px] border-orange-500 shadow-sm border-y border-r border-slate-100 group transition-all hover:shadow-md">
      ...
      <div className="bg-orange-50 p-8 rounded-[3rem] border border-orange-100 text-center relative overflow-hidden">
        <div className="absolute top-0 right-0 p-2 opacity-10"><MessageCircle size={60}/></div>
        <p className="text-[11px] text-orange-800 font-bold italic leading-relaxed relative z-10">
